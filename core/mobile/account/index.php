...
class Index_ChingLeeingPage extends MobilePage
{
    public function login()
    {
        global $_W;
        global $_GPC;
        if (is_weixin() || !(empty($_GPC['__ching_leeing_member_session_' . $_W['uniacid']]))) {
            header('location: ' . mobileUrl());
        }
        if ($_W['ispost']) {
            $mobile = trim($_GPC['mobile']);
            $pwd = trim($_GPC['pwd']);
            $member = pdo_fetch('select id,openid,mobile,pwd,salt from ' . tablename('ching_leeing_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
            if (empty($member)) {
                show_json(0, '用户不存在');
            }
    ...
    protected function rf($type)
    {
        ...
        if ($_W['ispost']) {
            ...
            if (empty($member)) {
                pdo_insert('ching_leeing_member', $data);
            } else {
                pdo_update('ching_leeing_member', $data, array('id' => $member['id']));
            }
            if (p('commission')) {
