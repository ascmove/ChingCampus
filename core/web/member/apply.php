...
class Apply_ChingLeeingPage extends WebPage
{
    public function main()
    {
        ...
        $psize = 20;
        $condition = ' and dm.uniacid=:uniacid and dm.serviceapplied=1';
    ...
    public function audit()
    {
        ...
        $op = ($_GPC['op'] == 'confirm')?2:0;
        if ($_W['ispost']) {
            $rejectreason = trim($_GPC['rejectreason']);
            if ($profile['serviceapplied'] == 1) {
                if($op == 2){
                    m('notice')->sendAuditMessage($profile['openid'],'done');
                }else{
                    m('notice')->sendAuditMessage($profile['openid'],'fail',$rejectreason);
                }
                pdo_update('ching_leeing_member', array('serviceapplied'=>$op),array('id'=>$id));
                show_json(1, array('url' => referer()));
            }
            if ($profile['serviceapplied'] == 0) {
                show_json(1, array('url' => referer()),'未申请');
            }
            if ($profile['serviceapplied'] == 2) {
                pdo_update('ching_leeing_member', array('serviceapplied'=>$op),array('id'=>$id));
                show_json(1, array('url' => referer()),'已处理');
            }
