...
class Servant_ChingLeeingPage extends WebPage
{
    public function main()
    {
        ...
        $psize = 20;
        $condition = ' and dm.uniacid=:uniacid and dm.serviceapplied=2';
        ...
        foreach ($list as &$row) {
            ...
            $row['ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('ching_leeing_order') . ' where uniacid=:uniacid and servant_openid=:openid and status=4', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
            $row['ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('ching_leeing_order') . ' where uniacid=:uniacid and servant_openid=:openid and status=4', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
            $row['credit'] = m('member')->getCredit($row['openid'], 'credit1');
            $banlance = m('balance')->query($row['openid']);
            $row['banlance'] = $banlance['money'];
            unset($row);
        }
        if ($_GPC['export'] == '1') {
            ...
            if ($hascommission) {
                if (cv('commission.agent.edit')) {
                    ...
                    if (!(empty($adata))) {
                        ...
                        pdo_update('ching_leeing_member', $adata, array('id' => $id, 'uniacid' => $_W['uniacid']));
                        if (!(empty($agent_flag))) {
                            if (!(empty($member['agentid']))) {
            ...
            if ($hasglobonus) {
                if (cv('globonus.partner.check')) {
                    ...
                    if (!(empty($gdata))) {
                        ...
                        pdo_update('ching_leeing_member', $gdata, array('id' => $id, 'uniacid' => $_W['uniacid']));
                    }
                }
            }
            if ($hasauthor) {
                if (cv('author.partner.check')) {
                    ...
                    if (!(empty($author_data))) {
                        ...
                        pdo_update('ching_leeing_member', $author_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                    }
                }
            }
            if ($hasabonus) {
                if (cv('abonus.agent.check')) {
                    ...
                    if (!(empty($aadata))) {
                        ...
                        pdo_update('ching_leeing_member', $aadata, array('id' => $id, 'uniacid' => $_W['uniacid']));
                    }
                }
            }
            if ($hassns) {
                if (cv('sns.member.edit')) {
                    ...
                    if (!(empty($snsdata))) {
                        ...
                        pdo_update('ching_leeing_sns_member', $snsdata, array('id' => $sns_member['id'], 'uniacid' => $_W['uniacid']));
                    }
                }
            }
            show_json(1);
