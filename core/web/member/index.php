...
class Index_ChingLeeingPage extends WebPage
{
    ...
    public function query()
    {
        ...
        $ds = pdo_fetchall('SELECT * FROM ' . tablename('ching_leeing_member') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
        foreach ($ds as &$value) {
    ...
    {
        echo json_encode(array('ajaxmembergender' => $this->ajaxmembergender(), 'ajaxmemberlevel' => $this->ajaxmemberlevel(), 'ajaxprovince' => $this->ajaxprovince(), 'ajaxnewmember0' => $this->ajaxnewmember(0), 'ajaxnewmember1' => $this->ajaxnewmember(1), 'ajaxnewmember7' => $this->ajaxnewmember(7), 'ajaxnewmembertotal' => $this->ajaxnewmember(-1)));
    }
    ...
    protected function ajaxmembergender()
    {
        ...
        $sql_member = 'select gender,count(gender) as gender_num from ' . tablename('ching_leeing_member') . ' where uniacid = :uniacid group by gender';
    ...
    {
        global $_W;
        $levels = pdo_fetchall('select * from ' . tablename('ching_leeing_member_level') . ' where uniacid=:uniacid order by level asc', array(':uniacid' => $_W['uniacid']), 'id');
        $levelname = array();
        foreach ($levels as $l) {
        ...
        $levelname[0] = '普通等级';
        ksort($levelname);
        $sql_level = 'select level,count(level) as level_num from ' . tablename('ching_leeing_member') . ' where uniacid = :uniacid group by level';
    ...
    {
        global $_W;
        $province = pdo_fetchall('select province,count(province) as province_num from ' . tablename('ching_leeing_member') . ' where uniacid = :uniacid group by province', array(':uniacid' => $_W['uniacid']));
        $result = array();
    ...
    protected function ajaxnewmember($day = 0)
    {
        ...
        $member_count = pdo_fetchcolumn('select count(*) from ' . tablename('ching_leeing_member') . ' where uniacid=:uniacid', $param);
    ...
    protected function selectMemberCreate($day = 0)
    {
        ...
        $sql = 'select count(*) from ' . tablename('ching_leeing_member') . ' where uniacid = :uniacid and createtime between :createtime1 and :createtime2';
