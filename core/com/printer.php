...
class Printer_ChingLeeingComModel extends ComModel
{
    const PRINTRT_365 = 0;
    ...
    public function sendOrderMessage($orderid = 0, $verify = array())
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('ching_leeing_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
        if (empty($order)) {
            return;
        }
        $status = 0;
        ...
        $openid = $order['openid'];
        $order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('ching_leeing_order_goods') . ' og ' . ' left join ' . tablename('ching_leeing_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
        $goods = array();
        foreach ($order_goods as $og) {
            $goods[] = array('shorttitle' => (empty($og['shorttitle']) ? $og['title'] : $og['shorttitle']), 'goodsprice' => (double)$og['price'] / (int)$og['total'], 'goodstotal' => (int)$og['total'], 'goodstotalprice' => (double)$og['price'], 'optionname' => $og['optionname']);
        }
        ...
        if ($order['isverify'] && $order['verifystoreid'] && !(empty($verify))) {
            ...
            if ($order['verifystoreid']) {
                $store = pdo_fetch('select * from ' . tablename('ching_leeing_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $order['verifystoreid'], ':uniacid' => $_W['uniacid']));
                $StorePrinterSet = $this->getStorePrinterSet($store);
            }
        }
    ...
    public function getStorePrinterSet($store)
    {
        ...
        if (!(empty($store['order_printer']))) {
            $store['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('ching_leeing_member_printer') . ' WHERE uniacid=:uniacid AND id IN (' . $store['order_printer'] . ')', array(':uniacid' => $_W['uniacid']));
        } else {
    ...
    public function getPrinterSet()
    {
        ...
        if (!(empty($data['order_printer']))) {
            $data['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('ching_leeing_member_printer') . ' WHERE uniacid=:uniacid AND id IN (' . $data['order_printer'] . ')', array(':uniacid' => $_W['uniacid']));
        } else {
    ...
    public function printer($params, $templateid, $printerid, $type = 0)
    {
        global $_W;
        $this->type = $type;
        $printer = pdo_fetch('SELECT * FROM ' . tablename('ching_leeing_member_printer') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $printerid));
        if (empty($printer)) {
            return error(-1, '小票打印机未找到');
        }
        $template = pdo_fetch('SELECT * FROM ' . tablename('ching_leeing_member_printer_template') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $templateid));
        if (empty($printer)) {
    ...
    public function sendCashierMessage($log, $templateid, $printerid, $operator)
    {
        ...
        if (!(empty($log['isgoods']))) {
            $goodsAll = pdo_fetchall('SELECT g.title,g.goodssn,og.price,og.total FROM ' . tablename('ching_leeing_cashier_goods') . ' g LEFT JOIN ' . tablename('ching_leeing_cashier_pay_log_goods') . ' og ON g.id=og.goodsid WHERE og.cashierid=:cashierid AND og.logid=:logid', array(':cashierid' => $log['cashierid'], ':logid' => $log['id']));
            foreach ($goodsAll as $og) {
                $goods[] = array('goodstitle' => $og['title'], 'goodsprice' => (double)$og['price'] / (int)$og['total'], 'goodstotal' => (int)$og['total'], 'goodstotalprice' => (double)$og['price'], 'optionname' => '无');
            }
        }
        if (!(empty($log['orderid']))) {
            $order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('ching_leeing_order_goods') . ' og ' . ' left join ' . tablename('ching_leeing_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $log['uniacid'], ':orderid' => $log['orderid']));
            foreach ($order_goods as $og) {
                $goods[] = array('goodstitle' => (empty($og['shorttitle']) ? $og['title'] : $og['shorttitle']), 'goodsprice' => (double)$og['price'] / (int)$og['total'], 'goodstotal' => (int)$og['total'], 'goodstotalprice' => (double)$og['price'], 'optionname' => $og['optionname']);
            }
        }
