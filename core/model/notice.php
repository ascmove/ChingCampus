...
class Notice_ChingLeeingModel
{
    public function sendwithdrawokMessage($openid = '',$string,$money,$uniacid){
        global $_W;
        $_W['uniacid'] = empty($uniacid)?$_W['uniacid']:$uniacid;
        $tm = m('common')->getSysset('notice',$_W['uniacid']);
        if($tm['withdrawok_close_normal'] == 0){
            $message = array(
                'first' => array(
                    'value' => $string,
                    'color' => '#000000'
                ...
                'timet' => array(
                    'value' => date("m余额d日 H:i"),
                    'color' => '#000000'
                ),
                'remark' => array(
                    'value' => "\n如有疑问或需要帮助请直接拨打我们的客服电话：".$_W['apptel'],
                    'color' => '#000000'
                )
            );
            $url = $this->getUrl('member');
            $account = m('common')->getAccount();
            $ret = m('message')->sendTplNotice($openid, $tm['withdrawok'], $message, $url, $account);
        }
        return;
    }

    public function sendorderexpireMessage($openid = '',$string,$money,$servicetypename,$serviceabstract,$outTradeNo,$uniacid){
        global $_W;
        $_W['uniacid'] = empty($uniacid)?$_W['uniacid']:$uniacid;
        $tm = m('common')->getSysset('notice',$_W['uniacid']);
        if($tm['orderexpire_close_normal'] == 0){
            $message = array(
                'first' => array(
                    'value' => $string."服务类型：".$servicetypename."\n服务内容：".$serviceabstract."\n退款金额：".$money.' 元',
                    'color' => '#000000'
                ),
                'keyword1' => array(
                    'value' => $outTradeNo,
            ...
            $url = $this->getUrl('order/detail',array('outTradeNo'=>$outTradeNo));
            $account = m('common')->getAccount();
            $ret = m('message')->sendTplNotice($openid, $tm['orderexpire'], $message, $url, $account);
        }
        return;
    }

    public function sendRefundMessage($openid = '',$money,$servicetypename,$serviceabstract,$outTradeNo,$uniacid){
        global $_W;
        $_W['uniacid'] = empty($uniacid)?$_W['uniacid']:$uniacid;
        $tm = m('common')->getSysset('notice',$_W['uniacid']);
        if($tm['refund_close_normal'] == 0){
            $message = array(
                'first' => array(
                    'value' => "服务类型：".$servicetypename."\n服务内容：".$serviceabstract,
                    'color' => '#000000'
                ),
                'keyword1' => array(
                    'value' => $outTradeNo,
            ...
            $url = $this->getUrl('order/detail',array('outTradeNo'=>$outTradeNo));
            $account = m('common')->getAccount();
            $ret = m('message')->sendTplNotice($openid, $tm['refund'], $message, $url, $account);
        }
        return;
    }

    public function sendTakeOrderMessage($openid = '',$servicetypename,$serviceabstract,$realname,$mobile,$outTradeNo){
        global $_W;
        $_W['uniacid'] = empty($uniacid)?$_W['uniacid']:$uniacid;
        $tm = m('common')->getSysset('notice',$_W['uniacid']);
        if($tm['takeorder_close_normal'] == 0){
            $message = array(
                'first' => array(
                    'value' => "如有其它具体需求，可直接联系服务者。\n服务类型：".$servicetypename."\n服务内容：".$serviceabstract,
                    'color' => '#000000'
                ),
                'keyword1' => array(
                    'value' => $realname,
            ...
            $url = $this->getUrl('order/detail',array('outTradeNo'=>$outTradeNo));
            $account = m('common')->getAccount();
            $ret = m('message')->sendTplNotice($openid, $tm['takeorder'], $message, $url, $account);
        }
        return;
    }

    public function sendNewOrderMessage($openid = '',$servicetypename,$time,$school,$money,$orderinfo,$outTradeNo,$uniacid){
        global $_W;
        $_W['uniacid'] = empty($uniacid)?$_W['uniacid']:$uniacid;
        $tm = m('common')->getSysset('notice',$_W['uniacid']);
        if($tm['neworder_close_normal'] == 0){
            $message = array(
                'first' => array(
                    'value' => "您有新的服务订单:\n\n服务类型:".$servicetypename,
                    'color' => '#000000'
                ...
                'remark' => array(
                    'value' => "\n点击此处查看服务订单详情\n如需暂时关闭订单推送,请点击[推送设置]菜单",
                    'color' => '#000000'
                )
            );
            $url = $this->getUrl('order/detail',array('outTradeNo'=>$outTradeNo));
            $account = m('common')->getAccount();
            $ret = m('message')->sendTplNotice($openid, $tm['neworder'], $message, $url, $account);
            if(is_error($ret)){
                return false;
            }else{
                return true;
    ...
    public function sendAuditMessage($openid = '',$status = '',$reason = '证件不合格'){
        global $_W;
        $tm = m('common')->getSysset('notice');
        if($tm['audit_close_normal'] == 0){
            switch ($status){
                case 'done':
                    $message = array(
                        'first' => array(
                            'value' => '恭喜您审核通过，请在使用中遵循平台运营规范！',
                            'color' => '#000000'
                        ),
                        'keyword1' => array(
                            'value' => '通过',
                    ...
                    $url = $this->getUrl('service/result/done');
                    break;
                case 'fail':
                    $message = array(
                        'first' => array(
                    ...
                    $url = $this->getUrl('service/result');
                    break;
                default:
                    $message = array(
                        'first' => array(
                            'value' => '请您耐心等待审核结果，我们将会尽快审核您的信息。',
                            'color' => '#000000'
                        ),
                        'keyword1' => array(
                            'value' => '审核中',
                    ...
                    $url = $this->getUrl('service/apply');
            }
            $account = m('common')->getAccount();
            $ret = m('message')->sendTplNotice($openid, $tm['audit'], $message, $url, $account);
        }
        return;
    }
    ...
    public function sendTeamMessage($orderid = '0', $delRefund = false)
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('ching_leeing_groups_order') . ' where id=:id limit 1', array(':id' => $orderid));
        if (empty($order)) {
            return;
        }
        $openid = $order['openid'];
        ...
        $order_goods = pdo_fetch('select * from ' . tablename('ching_leeing_groups_goods') . ' where uniacid=:uniacid and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => intval($order['goodid'])));
        ...
        if ($order['status'] == 1) {
            if ($order['success'] == 1) {
                $order = pdo_fetchall('select * from ' . tablename('ching_leeing_groups_order') . ' where teamid = :teamid and success = 1 and status = 1 ', array(':teamid' => $order['teamid']));
    ...
    protected function getUrl($do, $query = NULL)
    {
        ...
        if (strexists($url, '/addons/ching_leeing/')) {
            $url = str_replace('/addons/ching_leeing/', '/', $url);
        }
    ...
    public function sendGroupsNotice(array $params)
    {
        ...
        if ($tm['is_advanced']) {
            ...
            if (!(empty($templateid))) {
                $advanced_template = pdo_fetch('select * from ' . tablename('ching_leeing_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $templateid, ':uniacid' => $_W['uniacid']));
    ...
    public function sendOrderMessage($orderid = '0', $delRefund = false)
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('ching_leeing_order') . ' where id=:id limit 1', array(':id' => $orderid));
        if (empty($order)) {
            return;
        }
        $openid = $order['openid'];
        ...
        $order_goods = pdo_fetchall('select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('ching_leeing_order_goods') . ' og ' . ' left join ' . tablename('ching_leeing_goods') . ' g on g.id=og.goodsid ' . ' where ' . $scondition . ' and og.uniacid=:uniacid ', $param);
        $goods = '';
        ...
        if (!(empty($order['storeid']))) {
            if (0 < $order['merchid']) {
                $store = pdo_fetch('select * from ' . tablename('ching_leeing_merch_store') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
            } else {
        ...
        if (!(empty($order['address']))) {
            ...
            if (!(is_array($address))) {
                ...
                if (!(is_array($address))) {
                    $address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('ching_leeing_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $order['addressid'], ':uniacid' => $_W['uniacid']));
                }
            }
            if (!(empty($address))) {
        ...
        if ($delRefund) {
            ...
            if (!(empty($order['refundid']))) {
                $refund = pdo_fetch('select * from ' . tablename('ching_leeing_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));
                if (empty($refund)) {
                    return;
                }
    ...
    public function sendMemberLogMessage($log_id = '', $channel = 0)
    {
        global $_W;
        global $_GPC;
        $log_info = pdo_fetch('select * from ' . tablename('ching_leeing_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log_id, ':uniacid' => $_W['uniacid']));
        ...
        if ($log_info['type'] == 0) {
            ...
            if ($log_info['status'] == 1) {
                ...
                $this->sendNotice(array('openid' => $log_info['openid'], 'tag' => 'recharge_ok', 'default' => $message, 'url' => $this->getUrl('home/balance'), 'datas' => $datas));
