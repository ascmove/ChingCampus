...
class Excel_ChingLeeingModel
{
    public function export($list, $params = array())
    {
        ...
        $excel = new PHPExcel();
        $excel->getProperties()->setCreator('芝麻校园')->setLastModifiedBy('芝麻校园')->setTitle('Office 2007 XLSX Test Document')->setSubject('Office 2007 XLSX Test Document')->setDescription('Test document for Office 2007 XLSX, generated using PHP classes.')->setKeywords('office 2007 openxml php')->setCategory('report file');
    ...
    public function import($excefile)
    {
        ...
        $path = IA_ROOT . '/addons/chinoldg_leeingold/data/tmp/';
        if (!is_dir($path)) {
            load()->func('file');
            mkdirs($path, '0777');
        }
