...
YDBOBJ.prototype.SetHeadBar = function (IntState) {
    if (this.isIos) {
        ...
        ifr = null;
        try {
            window.webkit.messageHandlers.SetHeadBar.postMessage(IntState)
        } catch (ex) {
        }
    } else if (this.isAndroid && window.App9vCom) {
...
YDBOBJ.prototype.UploadImage = function (receiveUrl, showFun, UserName, Key, IsCut, CutWidth, CutHeight, sourceType, isUpload) {
    if (this.isIos) {
        var ifr = document.createElement('iframe');
        if (undefined == sourceType) ifr.src = "app9vcom:##//UploadImage/?" + receiveUrl + "," + showFun + "," + UserName + "," + Key + "," + IsCut + "," + CutWidth + "," + CutHeight; else ifr.src = "app9vcom:##//UploadImage/?" + receiveUrl + "," + showFun + "," + UserName + "," + Key + "," + IsCut + "," + CutWidth + "," + CutHeight + "," + sourceType + "," + isUpload;
        ifr.style.width = "0";
        ifr.style.height = "0";
        document.body.appendChild(ifr);
        if (null != ifr) ifr.parentNode.removeChild(ifr);
        ifr = null;
        try {
            if (undefined == sourceType) window.webkit.messageHandlers.UploadImage.postMessage(receiveUrl + "," + showFun + "," + UserName + "," + Key + "," + IsCut + "," + CutWidth + "," + CutHeight); else window.webkit.messageHandlers.UploadImage.postMessage(receiveUrl + "," + showFun + "," + UserName + "," + Key + "," + IsCut + "," + CutWidth + "," + CutHeight + "," + sourceType + "," + isUpload)
        } catch (ex) {
        }
    } else if (this.isAndroid && window.App9vCom) {
        if (undefined == sourceType) window.App9vCom.UploadImage(receiveUrl, showFun, UserName, Key, IsCut, CutWidth, CutHeight); else window.App9vCom.UploadImage(receiveUrl, showFun, UserName, Key, IsCut, CutWidth, CutHeight, sourceType, isUpload)
    }
};
